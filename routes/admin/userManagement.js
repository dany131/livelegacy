    ...
    try {
        ...
        const userResponse = await user.save(
            async (error, documentSaved, numberOfRowsAffected) => {
                if (documentSaved) {
                    helper.sendEmail(user.email, "Account Details", user.firstName, `Your account has been set up using email: ${user.email} and password: ${userPassword}`);
                    res.status(200).json({ message: "User created successfully", data: userResponse });
                    return;
                } else {
...
router.post("/delete-user/:userId", auth, async (req, res) => {
    try {
        ...
        for (let i = 0; i < photos.length; i++) {
            fs.unlink(`${process.env.IMAGE_PATH}${photos[i]._doc.imageName}`, (error) => {
                // console.log(error);
            });
        }
        ...
        for (let i = 0; i < videos.length; i++) {
            fs.unlink(`${process.env.VIDEO_PATH}${videos[i]._doc.videoName}`, (error) => {
                // console.log(error);
            });
        }
