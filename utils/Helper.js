...
// Mailing function
async function sendEmail(email, subject, username, text, password) {
...
const videoUpload = multer({
  ...
  fileFilter(req, file, cb) {
    ...
    if (!file.originalname.match(/\.(mp4|MPEG-4|mkv)$/)) {
      return cb(
        new Error(
          "File type not supported (Supported types are mp4, MPEG-4, mkv)"
        )
      );
    }
    cb(undefined, true);
...
function isISODateFormat(date) {
  return /\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z)/.test(
    date
  );
}
...
async function sendEmailContactUs(
  email,
  subject,
  name,
  userEmail,
